...
if [[ -n "$DATE" ]]; then
    if [[ -n "$2" && -n "$3" ]]; then
        ...
        echo "Creating GeoTiff for PRMSL data from $DATE (hours: $START_HOUR to $END_HOUR)"
    else
        echo "Creating GeoTiff for PRMSL data from $DATE (all hours)"
    fi
else
    echo "Creating GeoTiff for PRMSL data from all GRIB2 files in grib2/ directory"
fi
...
if [[ -n "$DATE" ]]; then
    if [[ -n "$2" && -n "$3" ]]; then
        ...
        for ((i=START_HOUR; i<=END_HOUR; i++)); do
            hour=$(printf "%03d" $i)
            file_pattern="grib2/prmsl_${DATE}_${hour}.grib2"
        ...
        # 特定の日付のすべてのファイル
        file_list=(grib2/prmsl_${DATE}_*.grib2)
    fi
else
    # すべてのGRIB2ファイル
...
for grib_file in "${file_list[@]}"; do
  if [[ -f "$grib_file" ]]; then
    ...
    # 入力GRIB2ファイルをGeoTiffに変換
    docker run --rm -v "$PWD":/work -w /work ghcr.io/osgeo/gdal:alpine-normal-latest \
      gdal_translate "$grib_file" "tif/${base_name}.tif"

    # Pa → hPa（/100）へ変換
    docker run --rm -v "$PWD":/work -w /work ghcr.io/osgeo/gdal:alpine-normal-latest \
      gdal_calc.py -A "tif/${base_name}.tif" --outfile="tif/prmsl_hpa_${base_name#prmsl_}.tif" \
      --calc="A/100.0" --type=Float32

    # 元のtifファイルを削除
    rm -f "tif/${base_name}.tif"
    
    echo "Created tif/prmsl_hpa_${base_name#prmsl_}.tif"
  fi
done

echo "All GeoTiff files for PRMSL created successfully!"