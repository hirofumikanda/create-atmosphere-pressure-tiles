...
show_usage() {
    ...
    echo "  STEPS: 実行ステップ（all|fetch|geotiff|isobar|terrainrgb、デフォルト: all）"
    echo ""
    echo "実行例:"
...
if [[ ! "$STEPS" =~ ^(all|fetch|geotiff|isobar|terrainrgb)$ ]]; then
    echo "エラー: STEPSは all|fetch|geotiff|isobar|terrainrgb のいずれかを指定してください"
    show_usage
    exit 1
fi

echo "========================================"
echo "気圧データ処理パイプライン開始"
echo "日付: $DATE"
...
if [[ "$STEPS" == "all" || "$STEPS" == "fetch" ]]; then
    run_step "データ取得" "01_fetch_prmsl.sh"
fi
...
if [[ "$STEPS" == "all" || "$STEPS" == "isobar" ]]; then
    run_step "等圧線作成" "03_create_isobar.sh"
fi

if [[ "$STEPS" == "all" || "$STEPS" == "terrainrgb" ]]; then
    run_step "TerrainRGB作成" "03_create_prmsl_terrainRGB.sh"
fi

echo ""
echo "========================================"
echo "気圧データ処理パイプライン完了"
echo "========================================"